...
//NOTICE: Prototyping different ideas for interaction with smart contracts
...
contract TestContract {
    ...
    function callSell(uint _cup, uint _price) public pure returns(bytes memory) {
        bytes memory data = abi.encodeWithSignature("marketSale(uint256,uint256)", _cup, _price);
        
        return data;

    }
    
    function callM() public pure returns(bytes memory) {
        bytes memory data = abi.encodeWithSignature("callsMarketplace()");
